...
// 클래스 입력한 후 JSON 데이터 붙여넣기 {} 만 자동으로 완성

// 2번 방법 :
// Dart Data Class Generator 플러그인
// 필드만 작성 후,
// (window 기준) Alt + insert   (Mac) Cmd + N
...
class Post {
  ...
  // Dart Data Class Generator -> 인텔리제이에서 자동으로 사용하는 플러그인 설정
...
class Post {
  ...
  //   toJson 은 어플에서 백엔드로 데이터를 저장할 때 사용하는 방식
  factory Post.fromJson(Map<String, dynamic> json) {
    return Post(
      postId: json['postId'],
      userId: json['userId'],
  ...
  // 매번 json['postId'] 와 같이 web 에서 가져온 json 형식에서 'postId' 라는 명칭으로 지칭된
  // 데이터를 사용하겠다. 모든 변수이름 앞에 json 을 붙이고 []로 표현
  // 직접으로 길게 작성하고 싶지 않기 때문에
  // 백엔드에서 앱으로 가져온 json 형태를 postId 라는 변수 명칭에 대입하여
  // jsonp[] 형식이 없어도 데이터를 표현 가능하도록 설정
