...
import 'package:flutter_workspace/basic/count_screen.dart';
import 'package:flutter_workspace/basic/for_screen.dart';
import 'package:flutter_workspace/basic/if_screen.dart';
import 'package:flutter_workspace/basic/while_screen.dart';
 ...
 * Private(_route) : 같은 파일 내에서만 접근 가능
 ...
 *
 * GoRouter 설정
 * GoRoute 생성자를 이용해서 객체를 생성하고
 * _route 변수이름에 저장
 */
// 아래 하위 코드에서 _router 내부에 존재하는 데이터 수정 불가! 상수처리하고
// 외부에서 직접적으로 접근할 수 없도록 비공개 처리한 private 변수
/*
아래 _router 와 route 변수는 각각 다른 상수 처리
...
_router 변수는 내부 코딩에서만 사용 가능한 비공개 상수
*/
...
final GoRouter _router = GoRouter(
  ...
  /*
    ...
    builder 이런 화면이 보일거에요
    context = App 에서 각 화면이 어디에 존재하는지 알려주는 주소
    state   = URL 에서 가져온 추가 정보들을 담고 있는 변수 = 보통은 null 상태
