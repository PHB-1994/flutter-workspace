...
class CountScreen extends StatefulWidget {
  ...
  CountScreen = CountScreen 이라는 위젯의 상태
  CountScreen 의 위젯 상태를 담당하는 클래스

  createState() = 상태를 만드는 함수

  flutter 가 변수 관리와 화면 그리기를 분리하여 앱 관리를 유용하게 할 수 있도록 하는 기능
  State<CountScreen> createState() => CountScreenState(); 필수가 아니고
  한 번에 하나의 class 에서 작성 가능하나 추후 에러 발생
  변수 데이터 로직 꼬임, 변수와 화면을 분리함으로써
  매번 화면 ui 를 호출해야하는 상황이 발생한다.
  하지만 현재와 같이 상태와 ui 를 분리하는 코드를 작성하면
  화면은 그대로 존재하는채로 데이터만 변경되고, 변경된 데이터가 반영된 상태로 화면 유지
   */
  State<CountScreen> createState() => /* return 생략되어 있음 */_CountScreenState();
}
...
class _CountScreenState extends State<CountScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      // 각 상단바에는 title 을 이용해서 어떤 페이지인지 표기
      appBar: AppBar(
        title: const Text("숫자 증가 감소 초기화"),
        ...
        child: Column(
          ...
          children: [
            ...
            ElevatedButton(
                ...
                // 다수의 코드 작성의 경우 {} 필히 작성 후 내부에 코드 작성
                onPressed: () {
                  setState(() {
